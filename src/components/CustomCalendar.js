            ...
            <div className="calendar__body">
                ...
                <div className="calendar__dates">
                    {fullDate.map((item, index) =>
                        <div key={index}>
                            <div className={"calendar__date " + (item.today ? "calendar__date--selected" : (item.siblingMonth ? "calendar__date--grey" : ""))} onClick={ () =>{ props.onSetDate(String(item.day).padStart(2, '0') + "/" + String(selectedMonth + 1).padStart(2, '0') + "/" + selectedYear)}}>{item.day}</div>
                        </div>
                    )}
                </div>
            </div>
